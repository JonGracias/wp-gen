...
function configure_wordpress.configureWordpress(globals)
    ...
    if not utils.doesDirectoryExist(wp_content .. '/wp-content') then
        ...
        utils.exec_command(string.format("mkdir -p %s", wp_content),
            nil, 'Error: configure_wordpress.configureWordpress(globals)')

        permissions.setAllPermissions(wp_content)

        local move_wp_content = string.format("mv %s/wp-content %s/", wp_base, wp_content)
    ...
    local create_symlink = string.format("ln -sf %s/wp-content %s/wp-content", wp_content, wp_base)
...
define('WP_SITEURL', '%s');
?>
]], db_name, db_user, db_password, db_host, wp_content, site_url, site_url)
